...
class Sam3VideoPredictor:
    ...
    def add_prompt(
        ...
        if len(result) == 2:
            frame_idx, outputs = result
        elif len(result) == 4:
            # Instance point refinement returns native tracker masks rather than
            # the detector-style output dictionary used by this server wrapper.
            # Normalize it here so add_prompt has one stable public response.
            frame_idx, obj_ids, _, video_res_masks = result
            masks = video_res_masks[:, 0] > 0 if video_res_masks is not None else None
            boxes = []
            probs = []
            kept_obj_ids = []
            kept_masks = []
            if masks is not None:
                ...
                for native_id, mask in zip(obj_ids, masks):
                    ys, xs = mask.nonzero(as_tuple=True)
                    if xs.numel() == 0:
                        continue
                    x1, x2 = xs.min().item(), xs.max().item() + 1
                    y1, y2 = ys.min().item(), ys.max().item() + 1
                    boxes.append([x1 / width, y1 / height, (x2 - x1) / width, (y2 - y1) / height])
                    probs.append(1.0)
                    kept_obj_ids.append(int(native_id))
                    kept_masks.append(mask.cpu().numpy())
            outputs = {
                "out_obj_ids": np.asarray(kept_obj_ids, dtype=np.int64),
                "out_probs": np.asarray(probs, dtype=np.float32),
                "out_boxes_xywh": np.asarray(boxes, dtype=np.float32).reshape(-1, 4),
                "out_binary_masks": np.asarray(kept_masks, dtype=bool),
            }
        else:
            raise RuntimeError(f"Unexpected add_prompt return length: {len(result)}")
